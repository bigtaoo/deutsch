...
（2026-09-03 起它还兼一件事：**替手机算对齐**，见上面「几件值得知道的事」。
那是唯一一处音频会离开设备的地方，且只到自己这台机器上，跑完即从内存里放掉、不落盘。）
...
**跨设备是自动的，两个方向都是**（2026-09-03 补上了「拉」那一半，SPEC FR-11.19）：
改过的东西自己推上去（标注 30s 去抖、复习会话结束立刻、设置 5s），
而**启动、回前台、刚连上网各自动拉一次** —— 只拉版本号变过的文档，所以常态下就是一次列表请求。
在桌面上导入并对齐一课，拿起手机就在那里，带句级和词级时间戳，一个按钮都不用点。
拉回来的东西照 §2.4 合并，**本地更新的那部分不会被覆盖**（会反过来推上去）。
远端删掉的课**不会**自动删本机 —— 删除只在你动手的那台设备上发起。

同步状态常驻在首页顶部：登录的账号、**上次推送**、**上次拉取**、待推送项数。
静默失败的备份比没有备份更危险，所以它不是「出事才亮的灯」。
推和拉分两行显示是有意的：推通了不代表拉通了，而拉悄悄停掉的样子
（「另一台设备上的东西一直不来」）和「我最近没在别的设备上练」长得一模一样。

### 换设备 / 数据没了怎么办

设置页 →「从同步服务器恢复」→ Google 登录 → 一键恢复。
恢复回来的是标注层（连词级时间戳和还没接受的候选词都在），课程会显示「素材未下载」，
**点开哪一课就自动补齐哪一课**（DW 来源；音频照记着的直链取，文稿照页面地址取，一课 6–10 MB，
下载中横幅一直在）。想一次补完几课就去「来源」页逐课点。
手动导入的课程无从自动补齐，要重新选本地文件（应用会按时长校验，对不上会警告时间戳可能失效）。
绑回的是同一份音频就不会重新对齐 —— 时间戳是同步来的那一份。教材这类按组导入的课，
在课程列表的组头「一次选齐音频」，把整本书的音频文件全选上，每课按自己记着的文件名认领。

**教材**（比如 Aspekte neu）：课程列表 →「导入教材」→ 把出版社的 Transkript 整份粘进去
（从 PDF 全选复制即可，断行和页码会自动整理），按章勾题，再一次选好这几题的音频。
一题跨几轨就把那一题的「轨」调成几，几轨会按顺序拼成一个音频。文稿与音频只存在本机，别分享出去。
...
界面上一共只有**三个活动**：**课程**（听）· **复习**（练）· **生词本**（词）。
手机上它们是底部那三个标签，桌面上排在顶栏。来源、素材、记录、设置是管道，收在右上角的「⋯」里。
（导航、状态语言、排版与配色的规范写在 `SPEC.md` §12。）

```
① 导入   从 DW 自动导入，或粘贴文稿 + 选本地 mp3 —— 保存后直接落在「切句」
    ↓
② 切句   自动切分 → 扫一遍 → 合并/拆分误切 → 排除非朗读段落
         （一次性的准备工作，之后它收在课程页的「⋯」里，不占 tab 位）
    ↓
③ 通听   打开一课的落地页。完整听一遍，文本默认折叠；展开后逐词高亮（点词从那里播）
    ↓
...
⑦ 听写   对挖空句作答，四级判定；答错自动进复习队列
    ↓
⑧ 复习   手机上刷 FSRS 队列：**一个词两张卡** —— 听卡（只听声音）与读卡（只看文字），
         都是四选一、答对自动过、答错才亮卡背
```
...
复习的题面一律是**德语释义** —— 词典里的中译只覆盖六成，凑不齐一道四选一。
想要中文的话，生词本页底部有一块**补中译**：复制词表 → 在外面翻 → 按编号粘回来
（和逐句译文同一条路子，这个应用自己不翻译）。补齐之前中文只出现在答错后的卡背上。

②③④⑤ 顺序可打乱；⑥⑦ 依赖 ④；⑧ 依赖 ⑤ 或 ⑦。

### 译文与笔记

**译文**：课程页右上角「⋯」→ **译文**。这个应用自己不翻译，它只负责把你在外面译好的
一整篇中文切回一句一句：

1. 点「复制原文与提示词」，粘给 Claude 之类的工具；
2. 把译好的结果粘回来 —— **编号是唯一的依据**（提示词里已经要求保留编号），
   所以译文里的编号乱了或没了，这里宁可少认几句，也不会按行数去猜；
3. 保存前有一屏原文/译文对照，扫一眼再存。

存好之后，通听（展开文本）和跟读上会多一个**「显示中文」**开关，默认关着 ——
和文本默认折叠是同一个道理，先用耳朵。一篇太长可以分两次贴，第二次不会把第一次的清掉。

**笔记**：通听页底部那个「笔记」折叠块，一课一块纯文本，边写边存。
译文和笔记都跟着这一课备份和同步，也都不会出现在「可分享包」里。

### 学习记录与分享图

「⋯」→ **记录**：今天学了多久、连续几天、一共几天，加最近两周的柱子；
下面是一张 1080×1350 的分享图（照片底 + 一句公有领域的德语谚语/引文 + 中译 +
你的名字、日期和那三个数），可以直接存图或拉系统分享面板。

计时只在**练习界面**（通听/跟读/学词/听写/复习）上走，而且要求页面在前台、
最近一分钟内点过屏幕或键盘、或者音频正在播 —— 切出去、锁屏、暂停着放一边，表都会停。
一天满 60 秒才算「学过的一天」。

**记录从 2026-09-15 开始攒，没有回填。** 以前的学习没有留下逐日痕迹
（FSRS 只记每张卡最后一次复习的时刻），硬凑出来的天数会是假的。
记录跟着账号同步，两台设备同一天的时长会加起来。

几件值得知道的事：

- **下载完就自动对齐，不用点任何按钮。** 音频一到位（DW 导入、手动导入、补齐素材、重新绑定音频）
  就排队自动对齐，进度常驻在**应用底部** —— 它要跑几分钟，所以你可以切去别的页面，
  甚至去复习，回来看一眼就行。想停就点那条上的「停止」。
- **在哪儿算：桌面上是本机，手机上只有服务器**（2026-09-21，SPEC FR-15.18）。桌面一课 44 秒，
  没理由上传；**手机上没有本地这条路了** —— 它从来没真的跑完过一课，却要随包背 418MB 权重，
  2026-09-21 整条删掉。登录之后手机把音频（6–10MB）送到自己那台服务器算，几分钟回来，
  **这期间手机可以锁屏、可以退出 App**。**只有音频上去，文稿不上去**（对齐里认声音那一半
  压根不看文本）。没登录、或服务器关掉了对齐，手机上就不对齐 —— 那时正确的做法是在桌面上对一次，
  句级和词级时间戳都会跟着同步回来。桌面上另有一个 ghost 按钮「送到服务器算」，用来对照两边。
- **对齐给的是「待校对」，不是「完成」。** 它在你本机跑 CTC 强制对齐（音频不出设备），
  然后把置信度明显偏低的几句在课程页头部报出来。**2026-09-21 换成德语原生模型之后**
  （原来那个是多语言罗马化的，德语的 ä/ö/ü/ß 进去之前全被抹平），同一课的置信度中位数
  从 -1.1 上下变成 **-0.02**，报出来的从 4 句变成 3 句 —— 而那 3 句全是采访同期声，
  也就是真的不好念的地方，不再是「模型看不懂的字符」。**报 0 句是可能的，也是诚实的**。
  仍然会漂的是：台标音乐、播音员交替、以及数字（`zwischen 18 und 30` 里的数词对齐时被丢掉了）。
- **手工打点界面已经删掉了**（「标注」tab 一起没了）。某一句真的对偏了，只能整课「重新对齐」，
  或者在桌面上跑一次 —— 句级时间戳跟着备份同步回来。老数据里手打过的点**不会**被重跑覆盖。
- **首次使用要下载一次约 230 MB 的模型**（从自己那台服务器的权重站取，不是 Hugging Face ——
  那份 4-bit 权重是自己量化的），之后进 Cache API、离线可用。Android 壳里权重随包带。
...
- **一课的 tab 顺序就是动线**（通听 → 跟读 → 学词 → 听写），tab 名旁边那个点说的是
  「前提够不够」而不是「做完没做完」：通听和跟读要时间戳，听写还要挖空。
...
- **同一个词的不同变形不会变成两个词条。** `laufen` 和 `gelaufen` 归到同一个词元，
...
- **听卡的正面只有声音，没有文字**（课程卡的挖空原句也不给 —— 给了文字就能靠语法猜，
  那就不是听力题了；看词选意思值得练，但那是**另一张卡**）。声音优先用 Wiktionary 上的真人录音（发卡时就下好，所以断网也能复习），
  没有录音的退到系统合成音，卡面会标出是哪一种。预置卡是**孤立词**发音 ——
  练不到连读，那仍然得靠课程里的真语料。
- **一个词有两张卡：听卡与读卡**（2026-09-21 加的，SPEC FR-21）。听觉识别和视觉识别
  是两套独立的记忆 —— 「听得出来但看到不确定什么意思」和「看得懂但听不出来」是两回事，
  混成一张卡就分不清自己错在哪一环。读卡在**听卡熟了之后**才自动开出来，
  而且**同一个词的两张卡同一天只出一张**（另一张是白送：答案几分钟前刚在眼前出现过）。
  读卡考三种：看词形选释义 → 看释义选词形 / **看句子挖空选词**（后两种交替）。
- **复习是四选一，而且不用你自己评分。** 听卡的新卡考「听音选词形」（四个选项是**音近词**，
...
**自动对齐的模型权重不属于内容。** 它是一份公开的声学模型二进制
（Apache-2.0 的德语 wav2vec2，浏览器那份是自己量化的 4-bit，230.2 MiB），
不是学习素材，所以不受 R-1 约束。权重要么随打包版带上（Android），要么首次使用时从
自己那台服务器的权重站取一次 —— 放在自己服务器上不是为了藏，是因为 Cloudflare 单文件
上限 25 MiB 放不下它，而 Hugging Face 上只有未量化的那份（1.2GB，浏览器跑不动）。
...
能看到当前跑的是哪一份。**需要重新出包的只有原生那半**：`AppDelegate`、`Info.plist`、
图标与启动图、新装的 Capacitor 插件。
...
**包不再大了**：0.2.1 的 IPA 曾是 **408MB**，其中 418MB 量级是对齐权重。
**2026-09-21（SPEC §0 变更 42）把 iOS 上的本地对齐整条删掉**，权重不再随包 ——
0.5.0 **实测装机 95.2MB**（剩下的大头是 40MB 词库），比 0.2.x 的 553.6MB 少了 458MB —— 83%。

**iOS 上的对齐只有服务器一条**：emissions 交给自己那台服务器算（`server/src/align/`），
手机只上传 mp3、下载矩阵，Viterbi 在手机上几秒钟。设置页「对齐后端」第一行会直接说
「这台设备不自己算 emissions」。**两条路用的不是同一份权重**（服务器 fp32、浏览器自己量化的 q4），
所以同一课换个地方重对，边界会有几十毫秒级的出入 —— 这是主动放弃的一致性，理由见 SPEC §0 变更 42。

**手机上对齐被系统杀掉时**：重开应用，顶部会有一条黄条说明上次死在哪一步、用的哪套后端 ——
这条信息来自边跑边落盘的黑匣子（`src/align/journal.ts`），设置页「对齐后端（FR-15 诊断）」里留最近几次。
下一次会自动换一档更保守的后端（`webgpu/q4` → `wasm/q4`，同一份权重换执行后端）；两档都被杀过就停掉自动对齐，
...
**只有 Android 那条**还会跑 `npm run stage:align`（带缓存）把 230MB 权重放进 `public/models/`，
所以 APK 装完第一次用就完全离线。iOS 那条 2026-09-21 起不带权重了（变更 42）。
...
- **手机上降档救不了 —— 而挪出 WebView 也没有救得了它。** iPhone 13 上 WebView 里两档都在
  加载模型时被杀，于是 2026-09-03 写了个原生 ONNX 插件把 emissions 挪出 WebView。
  它确实不再被杀了，但**换来的是另一种跑不完**：一课十几分钟满载、必须一直亮屏，
  而 iOS 三十秒就锁屏、锁屏即挂起。变更 33 先把它从自动降成手动，**2026-09-21（变更 42）
  整条删掉** —— 连同 418MB 随包权重、那个要联网 resolve 的 SwiftPM binaryTarget，
  以及 §7.12 里那条给 `/models/` 建符号链接的麻烦。手机上的对齐现在只有一条：服务器算。
  **留下的教训**：那道缝（`src/align/emissionMatrix.ts`）是对的，用它接服务器是对的；
  错的是「把同一个 300M 模型搬到手机的另一个进程里」这个想法本身 —— 换的是进程，
  不是这台设备的算力和电池。
