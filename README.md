...
| | 缓存层 | 标注层 |
|---|---|---|
| 内容 | **只有两样**：音频、原始文稿 | **其余全部**：时间戳（句级 + 词级）、切句边界、挖空、Glossar 候选词、生词、FSRS 状态、困难标记，以及音频与文稿的**下载地址** |
...
上传的是那一小份标注层：生词（含 FSRS 状态）和每一课的时间戳（句级 + 词级）/切句/挖空/候选词。
**音频和原始文稿一个字节都不上传** —— 上传的是它们的下载地址，换设备后照地址取回来。
设置目前也不上传（对齐档位这些跟机器走，要不要跨设备覆盖还没定，见 SPEC §0 变更 27 末尾）。
