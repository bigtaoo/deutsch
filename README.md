...
- **在哪儿算：桌面上是本机，手机上是服务器**（2026-09-03，SPEC FR-15.18）。桌面一课 26 秒，
  没理由上传；手机上本地要十几分钟、还得一直亮屏，所以登录之后手机会把音频（6–10MB）
  送到自己那台服务器算，一两分钟回来 —— **这期间手机可以锁屏、可以退出 App**，
  计算不在这台设备上。**只有音频上去，文稿不上去**（对齐里认声音那一半压根不看文本）。
  课程页上那两个按钮可以随时手动指定用哪一边；没登录、或服务器关掉了对齐，
  手机就回到「不自动对齐」那条老路（那时候正确的做法仍然是在桌面上对一次）。
...
**而登录之后手机上的默认路径又变了一次**（2026-09-03 晚，SPEC §0 变更 35）：
emissions 交给自己那台服务器算（`server/src/align/`），原生插件降级成「服务器不可用时的手动退路」。
设置页「对齐后端」第一行那时会说「emissions 由服务器算」。三条路（浏览器 / 原生 / 服务器）
用的是同一份 q4 权重、同一套分块规则，所以同一课在哪儿算都该得到同一份时间戳。
