...
//
// ── 为什么排除 dict/（和 models/）──
// 词库占 dist 的 40MB，而且**从不随代码改动而变**（scripts 生成之后基本不动）。
// 手机上它一直用 IPA 里那一份 —— 热更包解开后由 AppDelegate.swift 建符号链接指回去。
// `models/` 留在名单里是为了桌面构建：变更 42 之后 iOS 包里已经不再有对齐权重，
// 但本机跑过 stage:align 的 dist 里还会有，不排除的话热更包会一下子胖 400MB。
 ...
 *
 * **手改**。凡是这次前端改动用到了新的原生能力（新装的 Capacitor 插件、
 * AppDelegate 里的新行为），就把它提到那一版壳的版本号 —— 否则包会推给一个
