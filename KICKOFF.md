...
iOS 包应落到 **135MB 量级**。Android 那条**没动**（它仍然随包带权重）。
...
0h. **变更 42 剩的三步**（按顺序做，前两步不需要 Mac）。
   ...
   **fp32 一块几秒也量了**：加载权重 2.4 秒、满块（20 秒音频）**6.7 秒**、实时 **2.81×**
   ...
   ~~② 出一个 0.5.0 的 iOS 包~~ **✅ 2026-09-21 已出并上传 ASC**（`ios-v0.5.0`，构建号 10，
   run 35583768816，6m04s；`cap sync ios` 绿 = 插件删干净了）。**IPA 410.2 MiB → 32.7 MiB。**
   **剩真机那一步**：ASC 处理十几分钟之后从 TestFlight 装上去，看四件事 ——
   装机体积、课程页上没有「在这台手机上算」、登录后导入一课能自动送服务器算完、
   以及热更仍然生效（`/dict/` 那条符号链接现在是 `linkBundledAssets` 唯一还在链的东西）。
