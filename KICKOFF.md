...
**§9 实现顺序的 11 步已全部完成**，`main` 上可跑、可构建、**348 个单元测试通过**。
...
5. **在 iPhone 上确认预置卡到底有没有真人音**（见上）。设置页「词典」那一节会显示发音缓存
...
6. Q7：存档列表页 `/de/alltagsdeutsch/s-9214` 是否也在 `__APOLLO_STATE__` 里给出完整的 400+ 期列表。
