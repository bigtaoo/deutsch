...
**界面整体重做（2026-09-05，SPEC §0 变更 36 + 新增 §12「界面规范」）—— 代码已提交，但还在
分支 `ui/redesign-section-12` 上，没推、没合、没上线。** 线上 d.gamestao.com 跑的仍是旧界面。
...
0c. **界面重做那一支：合进 `main`、部署，然后在 iPhone 真机上看安全区。**
   分支 `ui/redesign-section-12` 领先 `main` 一个提交（`c62cff6`），**没有 upstream、没推过**，
   所以线上还是旧界面。合并之前没有别的活要做 —— 浏览器那一轮已经走完（见上面「现状」第一段）。
