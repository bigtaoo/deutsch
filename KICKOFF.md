...
**§9 实现顺序的 11 步已全部完成**，`main` 上可跑、可构建、**423 个测试通过**
（其中 `src/pages/ReviewPage.test.tsx` 是第一个组件测试 —— 复习页的时序行为只有装起来才测得到）。
...
**复习形式改成「听音四选一」，评分改成系统自动算**（2026-09-02，SPEC §0 变更 24，
...
- **预置词库从「加 10/25/50 个词」改成「报名一整档」**，卡片由复习页按 `newPerDay` 每天惰性激活。
  ...
  MediaWiki 往返和 100MB 以上录音，那条路走不通。
...
**这一批只在单测里验过（423 个全过 + `npm run build` 通过），没有在浏览器里点过，也没上手机。**
原因要记一下：`preview_start` 的 cwd 被钉在 `D:\deutsch`，而这次是在 worktree
（`.claude/worktrees/quiz-review`）里做的，所以起出来的 dev server 服务的是共享检出的代码。
合并回 `main` 之后要**亲手点一轮**，尤其是下面这两条。
...
而是 CC-BY-NC + Cloudflare Workers 跑不了（得上 VPS，那笔永久部署依赖正是 SPEC 第 209 行
否决 OAuth 代理的同一个理由）。要做的话正确切法是「只上行 mp3、只下行矩阵」。
