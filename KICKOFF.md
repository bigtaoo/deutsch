...
0h. **变更 42 剩的三步**（按顺序做，前两步不需要 Mac）。
   ...
   **剩下的两步**：
   ① **服务器真的算一课**（要登录，所以只能你来）：先
   `docker compose exec sync node src/align/probe.ts /data/probe25.wav` 读「fp32 一块几秒」
   —— 它会比 q4 那 4.1~4.5 秒慢，**慢多少是现在唯一完全未知的数**；
   再用桌面课程页那个 ghost 按钮「送到服务器算」对一课，顺带看 `docker stats` 的峰值，
   以及闲置 10 分钟之后那 1.3GB 有没有还回去。峰值撑不住就 `ALIGN_MODEL_DTYPE=q4` 退回去。
   ② **出一个 0.5.0 的 iOS 包**：验「安装包 ≈135MB」「课程页上没有『在这台手机上算』」
   「登录后导入一课能自动送服务器算完」。**热更不够** —— `AppDelegate` 变了（少链一条
   符号链接），而且要看的正是包的体积。
