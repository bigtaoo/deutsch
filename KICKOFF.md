...
**这一批只在测试里验过（合并后 447 个全过 + typecheck + `npm run build` 通过），
没有在浏览器里点过，也没上手机 —— 见「下一步建议」第 0 条。**
原因要记一下：`preview_start` 的 cwd 被钉在 `D:\deutsch`，而这批是在 worktree
（`.claude/worktrees/quiz-review`）里做的，所以在那里起出来的 dev server 服务的是共享检出的代码，
验不了 worktree 里的改动。替代手段是组件测试（`ReviewPage.test.tsx` 就是为此写的）。
**这批与同步那批是同一天并行的两摊活**，合并时撞了两处：变更表编号（我这条改成 29）
与 `ReviewPage.tsx` 里的 `backupVocabNow` → `syncVocabNow`。
...
0. **把新的复习流程亲手点一轮**（变更 29 那批只有单测）。三件事单测覆盖不到：
