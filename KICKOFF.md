...
0. **原生对齐插件在 iPhone 13 上跑一课 —— 这是现在唯一挡着 FR-15 收口的事。**
   包已经在 TestFlight 上（**0.2.1 构建号 7**，run 33750498587，IPA 408MB，第一个带插件的包）。
   三种结果对应三条路：
   ① **跑完了** → 去「设置 → 对齐后端」确认第一行是「emissions 由原生插件算：native / q4」、
   最近几次运行里那条是 `native/q4（原生插件，不在阶梯上）`，并记下一课实际跑多久。
   ② **报「算子找不到」** → ORT 的 iOS 分发里没有 `MatMulNBits`（`model_q4.onnx` 靠它，146 个节点），
   换 `bnb4`（212.2 MiB，走 DequantizeLinear + MatMul 那条标准路），改三处：
   `config.ts` 的 `NATIVE_PLAN.dtype`、`scripts/stage-align-assets.mjs` 的文件表、插件 README。
   ③ **还是被杀 / 一直不结束** → 顶部黄条 + 诊断页最近几次记录是唯一的死亡现场（`journal.ts`）。
   顺带：iOS 会在切后台后挂起进程，「对齐时别切走」目前**没有任何机制保证**（后台任务断言只有约 30 秒）。
