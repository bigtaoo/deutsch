...
0h. **变更 42 剩的三步**（按顺序做，前两步不需要 Mac）。
   **⚠️ 这三步都排在 `push main` 之前** —— 推 main 会同时发前端和后端，而权重还没上去时
   ...
   ① **服务器那一半上线**：`deploy/README.md` §4b 的五条命令（compose → 建目录 →
   送 q4 + 两个 json → 让服务器自己拉 fp32 → `up -d --build`），然后按「验收（对齐这一半）」
   跑 `probe.ts` 读「一块几秒」（fp32 会比 q4 那 4.1~4.5 秒慢，慢多少是这一步唯一的未知数），
   再 `curl -I` 一下权重站那个 URL。
   ② **权重站真的被用到一次**：把本机 `public/models/` 删掉、清掉浏览器的 Cache API，
   在 d.gamestao.com 上对一课 —— 第一次应该自己下那 230MB。
