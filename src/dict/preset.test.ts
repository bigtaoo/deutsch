...
describe('pickPresetWords', () => {
  ...
  it('一档取空了接着往下一档，不原地空转', async () => {
    const picks = await pickPresetWords(4, new Set(), 6, load);
  ...
  it('某一档取不到（没部署/断网）跳过它，不整个失败', async () => {
    ...
    const picks = await pickPresetWords(4, new Set(), 2, flaky);
    expect(picks.map((p) => p.w)).toEqual(['Umlaufbahn', 'Barmherzigkeit']);
  });

  it('所有档都取空了就返回已有的，不无限循环', async () => {
    const picks = await pickPresetWords(4, new Set(), 100, load);
    expect(picks).toHaveLength(6);
  });

  it('起始档超出上限时返回空', async () => {
    expect(await pickPresetWords(7, new Set(), 5, load)).toEqual([]);
  });
});
