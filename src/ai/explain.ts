...
//
// ── 两个入口，同一个函数 ──
// 查词面板（DictLookup）里两个词典都查不到时、以及生词本页每一行的「问 AI」按钮，
// 要的都是「给一段中文解释」。区别只在有没有 context（原句）/ existing（词典已给的释义），
// 由调用方决定传不传，这里不关心是从哪个入口来的。
//
...
// 不值得为它建一套专门的错误分类。唯一值得缓存的是「这台服务器没配 key」（下面的
// `serverSaidOff`），免得没配置的时候每次点都发一次注定失败的请求。
//
// ── 失败一律呈现成「AI 服务暂时不可用」──
