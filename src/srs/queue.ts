...
// 这原本是个近似（同一张新卡今天被 Again 反复重来会被算成复习），但它不需要任何
// 额外的每日计数器 —— 而每日计数器是要跨设备同步的，一同步就要处理时区和合并冲突，
// 为了一个「防爆闸」的软上限不值得。
//
// **FR-10.13 之后这个近似其实已经不成立了**：学习步骤关掉，同一张卡当天不会被评第二次
// （下面 `busy` 那道闸也不让它回来），于是 `reps === 1` 与「今天第一次学」严格等价。
// 注释保留原样是因为那个取舍的理由没变 —— 真要退回短期步骤，近似就又回来了。
