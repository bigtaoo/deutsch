...
export interface Lesson {
  ...
  /**
   * FR-20：这一课的笔记，纯文本一块，自己写的要点。
   *
   * **只做课程级，不做句级**：句级那一层已经被挖空和生词本占满了（两者都挂在句子上），
   * 再加一层句级笔记，过几周就想不起来某条东西当初记在哪一层。而「这篇在讲什么」
   * 本来就是关于整篇的。
   *
   * 和译文同为标注层、同样不进 `ShareablePackage` —— 笔记里大概率抄着原句。
   */
  notes?: string;
  createdAt: number;
...
export interface Sentence {
  ...
  words?: WordSpan[];
  /**
   * FR-19：这一句的中文译文。**不是机器翻译** —— 是人在外面（Claude 之类）译好之后
   * 按编号整篇粘回来的（`src/lesson/translation.ts`），应用自己不联网翻译。
   *
   * 挂在句子上而不是整课一段，是因为它的两个使用场景都是逐句的：通听展开文本时
   * 每行下面一行中文，跟读时当前句下面一行中文。整课一段译文要人在两块文本之间
   * 自己找位置，而听的时候手不在屏幕上。
   *
   * 在标注层（跟着 Lesson 走，备份 + 同步一行不用改）：它没有任何「下载地址」，
   * 丢了只能重译一遍 —— 按 §2.3 变更 27 的标准，这就是要备份的东西。
...
export interface VocabEntry {
  ...
  meaningZh?: string;
  /**
   * FR-9.11：**这个词要拿去问 AI**。用户在生词本里手点的标记。
   *
   * 值只是个标记（`true` 或者字段不在），不是队列位置 —— 顺序由 `pendingAi` 按
   * `createdAt` 算，那和「补中译」是同一套编号规则（FR-21.9）。
   *
   * 词典**查不到**的词不需要这个标记：它们由 `!meaning` 自动进待办
   * （FR-9.6 收下来的空卡，本来就只能靠外面补）。标记存在的意义是另一半 ——
   * 查到了、但那几个中文字看不出区别的那些词，机器分不出来，只能由人点。
   */
  askAi?: true;
  /**
   * FR-9.12：从 AI 会话里粘回来的详细解释。自由文本，可以有好几行。
   *
   * 和 `meaning`（德语释义，词典给的）、`meaningZh`（一句话中译，FR-21.9）
   * 分开存而不是并进 `meaning`：那两个字段有**结构上的用途** ——
   * `meaning` 要进读卡的四选一选项（截到 80 字符），`meaningZh` 要进卡背那一行。
   * 一段三百字的辨析塞进任何一个，都会把复习卡面撑爆。它只出现在卡背末尾和生词本里。
   */
  note?: string;
  createdAt: number;
  updatedAt: number;
...
export interface Settings {
  ...
  /**
   * FR-19.4：通听与跟读里是否显示中文译文。**默认关** —— 和 FR-5.1 让文本默认折叠
   * 是同一个理由，而且对译文只会更强：一眼看得到中文，耳朵就不必再努力了。
   *
   * 跟着人走（进 Settings 因而同步到所有设备），不跟着机器走：
   * 「今天想不想看中文」在桌面和手机上是同一个答案。
   */
  showTranslation: boolean;
  /**
   * FR-10.12：复习时的三个音效（点选项 / 答对 / 答错）。**默认开**。
   *
   * 给开关是因为这是**每答一题都会发生**的事：在图书馆或者身边有人睡觉时，
   * 唯一的出路不该是把整台手机静音（那会连德语音频一起关掉）。
   * 跟着人走进 Settings（因而同步），和 `showTranslation` 同一条理由。
   */
