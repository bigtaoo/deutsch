...
| FR-10.10 | **干扰项**：辨形题取同档 **IPA 最近邻**（构建时预算，FR-16.8）；辨义题取同档**同词性**随机 | 辨形题的干扰项必须音近，否则 `Zuversicht / Haus / gehen / rot` 这种题谁都不会选错，**辨音训练归零**，等于每天点十次「对」。辨义题反过来 —— 语义上太相近对使用者不公平。释义选项要截断到约 80 字符（`shortGloss()`）：词典里的德语释义是 Wiktionary 式完整定义，中位 60 字符、p90 126、最长 862，原样放进 2×2 网格会爆屏 |
...
| FR-16.8 | 牌组里每个词**预存 3–6 个同档 IPA 最近邻**，供 FR-10.10 的辨形题当干扰项 | 放构建期而不是运行时：档内两两比较是 O(n²)（band-4 是 3000²），构建期无所谓，手机上不可能。代价是牌组从 0.46 MiB 涨到约 1.3 MiB —— 在 35 MiB 的词典旁边是噪声。**没有 IPA 的词退到按词形相似**（band-4 实测 9/3000 缺 IPA） |
