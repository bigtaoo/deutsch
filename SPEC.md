...
| FR-10.4 | **评分由系统算，不让用户选**（`gradeFromAnswer()`）：选错或「不认识」→ `Again`；答对且 ≥4s → `Hard`；答对 <4s → `Good`；答对 <1.5s 且这张卡历史无错 → `Easy` | 原文那条「评分四档 `Again/Hard/Good/Easy`、键盘 `1234`、四个大按钮」**已废除**（变更 24）。两条理由：① 手评要求用户在每张卡上判断一次自己的记忆强度，那是元认知任务、不是学习任务；② 手评与自动评分喂给 FSRS 的分布不同，**两条路径不能并存**，混着用会让调度参数失真 —— 所以旧按钮是删掉，而不是留一个高级入口。界面上**不给可选间隔**，只在答对时如实告知「下次 X 后再见」（那是信息，不是选择） |
...
| FR-17.3 | **整档报名**：选中一档 → 「把这一档全部加入学习计划」（如第 4 档 3000 词）。面板显示「已激活 N / 剩余 M」 | 原文那条「加 N 个词（10/25/50）」**已废除**（变更 24）：它把「我要学哪些词」和「今天学几个」压成了同一个动作，于是每学完十几个词就要回去点一次按钮 |
| FR-17.4 | 报名**不等于建卡**。卡片由复习页**每天惰性激活**：今天新卡不足 `newPerDay` 就按名次补齐，顺手取这十几个词的发音，并在后台预取下一批 | **这是唯一能撑到 3000 词的形状。** 直接为整档建卡要 60 次 MediaWiki 往返（发音 50 词一批）+ 100 MB 以上的录音落库，那条路走不通。而「哪些还没学」**不必落库** —— 它等于「档内词表减去生词本里已有的词」，是个纯派生量（复用 FR-17.5 的词元键）。所以整档报名只写一条设置（`enrolledBands`），**不新增 store、不动 `DB_VERSION`** |
...
**第 13 步（2026-09-02 追加）：复习形式改成听音四选一（变更 24）。**
