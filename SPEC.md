...
| FR-15.3 | 结果一律呈现为**待校对**，不呈现为完成；低置信句在打点页标黄并排前 | 说成完成会让人跳过校对。阈值是**相对本课中位数**的，不是绝对值 —— 见 §7.9 |
...
| FR-15.5 | 打点页里的任何一次手工改动，都把该句升为 `timingSource: 'manual'` 并清掉置信度；「清除时间戳」退回未标注 | 不升级的话，你亲手校准过的句子会永远留在待校对列表里，那个列表立刻不可信 |
| FR-15.6 | 全程在 **Web Worker** 里跑，主线程只负责解码音频与显示分阶段进度 | 一课在 WASM 后端要几分钟。放主线程等于 PWA 冻住几分钟，连进度条都刷不出来 |
| FR-15.7 | DW 自动导入后按 `Settings.autoAlignOnImport`（默认开）自动跑一遍；失败**不算导入失败** | 与 FR-13.9「抓到哪算哪」同一个道理：课程已经落库了，打点还能手动补，没有理由把好不容易抓下来的素材一起回滚 |
