...
**图标与启动图自己画**（`scripts/generate-icons.mjs`，`npm run icons`）。官方的 `@capacitor/assets` 依赖 sharp，而 sharp 要跑安装脚本编原生模块 —— 为画几十个 PNG 引入一条需要编译的依赖链，比自己多写一百行糟得多；这与 §7.6「不为一次性构建产物装依赖」是同一条判断。一个只在上传时才炸的坑：**iOS 的 AppIcon 不能带 alpha 通道**，App Store 校验看的是通道在不在而不是有没有透明像素，所以那一张必须写成 PNG color type 2，Xcode 本地构建完全不报错。 图案是**两点变音符 + 一条正弦波**（两点说「德语」，波说「声音」；单画波形跟播客、录音机的图标没有区别）。波的起伏数必须是**奇数且 ≥ 5**：只有 1.5 个周期时，中间那道下凹的弧配上头顶两点，整枚图标会被读成**一张脸**（两只眼睛 + 一张嘴）—— 第一版就是这么翻车的，SVG 草图上还看不出来，渲染成 512 才躲不开。画法从逐像素硬判断换成 **SDF + 覆盖率**（覆盖率 = 0.5 − 有符号距离 / 像素边长，每像素只算一次距离，比超采样又快又准）：矩形波形有没有锯齿看不出来，圆点和圆角必须抗锯齿。几何参数写在 `ART` 里、按 1024 设计稿坐标给，归一化到「紧贴墨迹的盒子」由脚本算 —— 这样各平台的 `padding` 恒定表示留白比例，换图案不用手算新数值。
