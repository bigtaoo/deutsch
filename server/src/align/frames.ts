...
//
// 这个文件是 `src/align/emissions.ts`（浏览器）的**第二份**逐条移植
// （变更 42 之前还有第三份：iOS 那个原生 ONNX 插件，已随本地对齐整条删掉）。
// 两份必须逐字等价：同一课在桌面和这台服务器上算出来的时间戳应该落在同一处 ——
// 权重不同（浏览器 q4、这里 fp32）已经带来几十毫秒的出入，分块和帧号再错一点
// 就会变成一个用户看不见、却能听出来的差别。
//
// **改任何一处都要改另一处**，这条写在两个文件里各一遍。两处的锚点：
